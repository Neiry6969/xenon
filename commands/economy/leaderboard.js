...
module.exports = {
    ...
    async execute(
        ...
        jsoncooldowns[interaction.user.id].rich = timpstamp;
        fs.writeFile(
            "./cooldowns.json",
        ...
        await Promise.all(
            interaction.guild.members.cache.map(async (member) => {
                ...
                let ginventoryData;
                try {
                    ginventoryData = await inventoryModel.findOne({
                        userId: id,
                    });
                    if (!ginventoryData) {
                        ginventoryData = null;
                ...
                if (!ginventoryData) {
                    ...
                    Object.keys(ginventoryData?.inventory).forEach((key) => {
                        if (ginventoryData?.inventory[key] === 0) {
                            ...
                            itemsworth =
                                itemsworth +
                                item.value * ginventoryData?.inventory[key];
                        }
                    });
                }
                ...
                return itemsworth > 10000 &&
                    id !== "847528987831304192" &&
                    ginventoryData
                    ? collection1.set(id, {
                          id,
                          itemsworth,
        ...
        embed
            .setColor("RANDOM")
            .setTitle(`${interaction.guild.name} Net Balance Leaderboard`)
            .setDescription(
                `${
                    leaderboard
                        ? leaderboard
        ...
        collector.on("collect", async (i) => {
            ...
            if (i.customId === "leaderboardmenu") {
                if (i.values[0] === "networth") {
                    ...
                    embed
                        .setTitle(
                            `${interaction.guild.name} Net Worth Leaderboard`
                        )
                        .setDescription(
                            `${
                                leaderboard
                    ...
                    embed
                        .setTitle(
                            `${interaction.guild.name} Net Inventory Worth Leaderboard`
                        )
                        .setDescription(
                            `${
                                leaderboard
                    ...
                    embed
                        .setColor("RANDOM")
                        .setTitle(
                            `${interaction.guild.name} Net Balance Leaderboard`
                        )
                        .setDescription(
                            `${
                                leaderboard
