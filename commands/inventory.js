...
module.exports = {
    ...
    async execute(message, args, cmd, client, Discord, userData) {
        ...
        if(target) {
            economyModel.findOne({ userId: target.id }, async(err, data) => {
                if(!data) {
            ...
            if(!userData.inventory) return message.reply("You got nothing in your inventory.");

            const mappedData = Object.keys(userData.inventory)
                .sort()
                .map((key) => {
                    if(userData.inventory[key] === 0) {
                        ...
                        const itemIcon = allItems.find((val) => (val.item.toLowerCase()) === key).icon;
                        return `${itemIcon} ${key} ── \`${userData.inventory[key].toLocaleString()}\``;
                    }
                }
                )
                .filter(Boolean)
            ...
            if(mappedData.length === 0) {
                ...
                const inventory = Object.values(userData.inventory).filter(Boolean);
                const invlength = inventory.length;
                const itemsperpage = 16;
