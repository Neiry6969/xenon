...
module.exports = {
  ...
  async execute(interaction, client, theme) {
    ...
    const t_inventory_fetch = await fetchInventoryData(options.user.user.id);
    const t_economyData_fetch = await fetchEconomyData(options.user.user.id);
    ...
    if (quantity === "max" || quantity === "all") {
      if (itemData && quantity) {
        if (
          ...
          quantity = Math.floor(inventoryData.inventory[itemData.item]);
        }
      } else if (quantity) {
      ...
      quantity = 0;
    } else if (letternumbers.find((val) => val.letter === quantity.slice(-1))) {
    ...
    async function fight() {
        ...
        .setFields(
          {
            ...
            value: `<:lifesaver:978754575098085426> ${bardisplay(
              Math.floor((fightgame_status.local.health / 100) * 100)
            )} **${
              fightgame_status.local.health
            }%**\n<:shield:1013876234138177586> ${bardisplay(
              Math.floor((fightgame_status.local.shield / 100) * 100)
            )} **${
              fightgame_status.local.shield
            }%**\n<:sword:1013883185337221151> ${bardisplay(
              Math.floor((fightgame_status.local.buff / 100) * 100)
          ...
          },
          {
            ...
            value: `<:lifesaver:978754575098085426> ${bardisplay(
              Math.floor((fightgame_status.target.health / 100) * 100)
            )} **${
              fightgame_status.target.health
            }%**\n<:shield:1013876234138177586> ${bardisplay(
              Math.floor((fightgame_status.target.shield / 100) * 100)
            )} **${
              fightgame_status.target.shield
            }%**\n<:sword:1013883185337221151> ${bardisplay(
              Math.floor((fightgame_status.target.buff / 100) * 100)
      ...
      fight_collector.on("collect", async (button) => {
        if (
          ...
          fight_msg.embeds[0].description = `${interaction.user} **VS** ${
            options.user
          }\n**Prize:** ${doubleprize_display}\n**Turn Ends:** <t:${Math.floor(
            (Date.now() + 60 * 1000) / 1000
          )}:R>`;
        ...
        if (button.customId === "shield") {
          ...
          let attack_final = Math.floor(
            attack_amount - 20 * (fightgame_status[waitingraw].shield / 100)
          );

          if (attack_final < 0) {
            if (fightgame_status[turnraw].health - attack_final < 0) {
            ...
            fightgame_status[turnraw].health -= Math.abs(attack_final);
          } else {
            if (fightgame_status[waitingraw].health - attack_final < 0) {
          ...
          fight_msg.embeds[0].fields[2].value = `${
            attack_final < 0
              ? `\`${
                ...
                } tried to attack, but took ${Math.abs(attack_final)}% damage\``
    ...
    async function target_confirmation() {
      ...
      const collector_target = confirmfight_msg_target.createMessageComponentCollector(
        {
          time: 20 * 1000
        }
      );
